...
{
    /// <summary>
    /// Summary description for getDetailCountAssy
    /// </summary>
    public class getDetailCountAssy : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            try
            {
                List<getDetailCountV2_Result> areasDetail = m_db.getDetailCountV2().ToList();


                    html += "<div id='tableInfoDet'><table id='tableSerialsDet' class='table display nowrap' style='font-size:11px;width:93%;'>";
                    ...
                    htmlBody += "<tbody>";
                    int i = 1;
                    foreach (getDetailCountV2_Result area in areasDetail)
                    {
                    //htmlBody += "<tr class='tablerows'>";

                        htmlBody += "<tr>";
                        htmlBody += "<td style='font-weight:bold;background:lightgray;'>" +
                        "<button type='button' class='btn butTable' data-toggle='modal' data-target='.bs-example-modal-xl' onclick = 'getDetailAreaAssy(" + area.IDAREA.ToString() + ")'>" + area.ZONE + "</button></td>";
                        //htmlBody += "<td style='font-weight:bold;background:lightgray;'><a href='#' onclick = 'getDetailAreaAssy(" + area.IDAREA.ToString() + ")'>" + area.ZONE + "</a></td>";
